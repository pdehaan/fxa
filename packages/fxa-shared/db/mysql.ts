...
/**
 * Shared Implementation of my SqlStore. This can be a place to put routines required by more than one package
 */
export class MysqlStoreShared extends MysqlStoreBase {
  constructor(options: mysql.PoolConfig) {
    super(options);
  }

  private static readonly QUERY_LIST_REFRESH_TOKENS_BY_UID =
  ...
  private static readonly QUERY_LIST_ACCESS_TOKENS_BY_UID =
  ...
  async getRefreshTokensByUid(uid: string) {
    const refreshTokens = await this._read(
      MysqlStoreShared.QUERY_LIST_REFRESH_TOKENS_BY_UID,
      [buf(uid)]
    );
  ...
  async getAccessTokensByUid(uid: string) {
    const accessTokens = await this._read(
      MysqlStoreShared.QUERY_LIST_ACCESS_TOKENS_BY_UID,
      [buf(uid)]
    );
